    ...
    <div>
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            ...
            <p class="text-slate-500 text-sm mt-1">Deine Auswertungen zu den Hofstede-Dimensionen</p>
          </div>
        ...
        <div v-if="loading" class="flex flex-col items-center justify-center py-16 text-slate-500">
          ...
          <p>Lade deine Umfragen…</p>
        </div>
        <div v-else-if="surveys.length === 0" class="bg-white rounded-2xl shadow-soft border border-slate-100 p-12 text-center">
          <div class="w-16 h-16 rounded-2xl bg-primary-100 text-primary-600 flex items-center justify-center text-3xl mx-auto mb-4">📋</div>
          <p class="text-slate-600 font-medium mb-2">Noch keine Umfragen</p>
          <p class="text-slate-500 text-sm mb-6 max-w-sm mx-auto">Starte deine erste Umfrage und sieh dir direkt deine Auswertung zu den Kulturdimensionen an.</p>
          <router-link
            to="/survey"
            class="inline-flex items-center gap-2 px-6 py-3.5 bg-primary-600 text-white font-semibold rounded-xl hover:bg-primary-700 shadow-card hover:shadow-card-hover transition"
          >
